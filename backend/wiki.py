...
def seasonal_alerts() -> list[dict]:
    month = datetime.now().month
    alerts: list[dict] = []
    if month in (3, 4, 5):
        alerts.append({"level": "warning", "disease": "早疫病", "message": "春播期温湿度波动大，早疫病进入易发窗口"})
        alerts.append({"level": "warning", "disease": "斑枯病", "message": "斑枯病风险上升，建议预防性用药"})
    if month in (6, 7, 8):
        alerts.append({"level": "danger", "disease": "黄化曲叶病毒病", "message": "夏季烟粉虱活跃，TYLCV 传播风险高"})
        alerts.append({"level": "warning", "disease": "细菌性斑点病", "message": "梅雨/台风季注意细菌性斑点病"})
    if month in (9, 10):
        alerts.append({"level": "warning", "disease": "晚疫病", "message": "秋季昼夜温差大，晚疫病需重点防控"})
        alerts.append({"level": "warning", "disease": "叶霉病", "message": "温室通风减少，叶霉病高发"})
    if month in (11, 12, 1, 2):
        alerts.append({"level": "warning", "disease": "灰霉病", "message": "冬季设施低温高湿，灰霉病风险增加"})
        alerts.append({"level": "warning", "disease": "白粉病", "message": "干燥温室注意白粉病监测"})
    return alerts


def register_wiki_routes(app) -> None:
    @app.get("/api/wiki/diseases")
    def wiki_list():
        return jsonify({"diseases": DISEASE_WIKI, "alerts": seasonal_alerts()})

    @app.get("/api/wiki/diseases/<disease_id>")
    def wiki_detail(disease_id: str):
        item = next((d for d in DISEASE_WIKI if d["id"] == disease_id), None)
        if item is None:
            return jsonify({"error": "未找到该病害"}), 404
        return jsonify({"disease": item})
